<template>
	<view class="content">
		...
		<view class="login">
			...
			<!-- <view class="input" v-if='subject_id'>
				<input type="text" v-model="subject_title" placeholder="请选择你的学科" placeholder-class="p-c"/>
			</view> -->
...
export default {
	...
	onLoad(options) {
		...
		this.$api
			...
			.then(res => {
				...
				this.school = res.data.school_id;
	...
	methods: {
		...
		getphone(e) {
			...
			let _this = this;
			_this.$api.get_mobile(data).then(res => {
		...
		bindgetuserinfo(e, i) {
			...
			uni.login({
				success: res => {
					...
					_this.$api.teacher_login(data).then(res => {
